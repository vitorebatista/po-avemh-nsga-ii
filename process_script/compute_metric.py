...
algs = ["avemh", "nsga2"]
alg_names = ["AVEMH", "NSGA-II"]
for prob_num, market in enumerate(markets):
    pf = np.genfromtxt("../dat/portef" + str(prob_num+1) +
                       ".txt")  # points on pf
    M, V = [], []
    ...
    for alg in algs:
        for i in range(rep):
            df = pd.read_csv("../tmp/{}/{}/{}.csv".format(market, alg, i+1),
    ...
    metric_names = ["GD", "Delta", "Spacing",
                    "MaxSpread", "IGD", "Hypervolume"]
    for metric, metric_name in zip(metrics, metric_names):
        metric = pd.DataFrame(metric).T
        metric.columns = alg_names
        metric.to_csv("./num_res/{}.{}.csv".format(market,
                                                   metric_name), index=False)
